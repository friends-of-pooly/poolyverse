...
/**
 * @title PoolyMorphStorage
 * @author Kames Geraghty
 */
contract PoolyMorphStorage is ERC721Storage {
  constructor(
    address _svgRender_,
  ...
  function _parseName(uint256 _tokenId) internal view override returns (string memory) {
    return string.concat("PoolyMorph #", Strings.toString(_tokenId));
  }

  function _parseDescription(uint256 _tokenId) internal view override returns (string memory) {
    return string.concat("Member #", Strings.toString(_tokenId), " of PoolyMorph.");
  }
}
